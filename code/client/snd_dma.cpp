...
*/
	...
	_strlwr(sfx->sSoundName);//force it down low
...
void S_SoundList_f( void ) {
	...
	if ( Cmd_Argc() == 2 ) 
	{
		if (!_stricmp(Cmd_Argv(1), "shouldbeMP3"))
		{
			bShouldBeMP3 = qtrue;
		}
		else
		if (!_stricmp(Cmd_Argv(1), "wavonly"))
		{
			bWavOnly = qtrue;
		}
		else
		{
			if (!_stricmp(Cmd_Argv(1), "1"))
			{
				iVariantCap = 1;
			}
			else
			if (!_stricmp(Cmd_Argv(1), "2"))
			{
				iVariantCap = 2;
			}
			else
			if (!_stricmp(Cmd_Argv(1), "3"))
			{
				iVariantCap = 3;
			}
		}		
			...
			if (iVariantCap >= 1 && iVariantCap <= 3)
			{
				...
				if (iStrLen > 2)	// crash-safety, jic.
				{
						...
						&& isdigit(c) && atoi(va("%c",c)) > iVariantCap)
					{
						...
						{
							if (!_stricmp(sFindName,sfx2->sSoundName))
							{
...
static void S_UpdateBackgroundTrack( void )
{
	...
	*/
	...
	else
	{
		...
		qboolean bShouldBeSilent = (psCommand && !_stricmp(psCommand,"silence"));
