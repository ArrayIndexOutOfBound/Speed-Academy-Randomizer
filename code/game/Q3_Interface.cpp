...
static void Q3_SetCinematicSkipScript( char *scriptname )
{
	...
	{
		if (strcmp(scriptname, "kor2/endgame_ds_3_skip") == 0 ||
...
void CQuake3GameInterface::FreeEntity( gentity_t *pEntity )
{
	...
	if VALIDSTRING( pEntity->script_targetname )
	{
		...
		entitylist_t::iterator it = m_EntityList.find( _strupr(temp) );
...
void CQuake3GameInterface::AssociateEntity( gentity_t *pEntity )
{
	...
	m_EntityList[ _strupr( (char *) temp ) ] = pEntity->s.number;
}
...
int 	CQuake3GameInterface::PlaySound( int taskID, int entID, const char *name, const char *channel )
{
	...
	else if ( _stricmp( channel, "CHAN_VOICE_GLOBAL" ) == 0 ) // this should broadcast to everyone, put only casue animation on G_SoundOnEnt...
	{
...
extern cvar_t	*g_char_skin_legs;
	case SET_WINTER_GEAR:	// Created: 03/26/03 by AReis.
	{
		...
		if ( entID == 0 || ( ent->NPC_type && _stricmp( ent->NPC_type, "player" ) == 0 ) )
		{
...
int		CQuake3GameInterface::GetByName( const char *name )
{
	...
	ei = m_EntityList.find( _strupr( (char *) temp ) );
