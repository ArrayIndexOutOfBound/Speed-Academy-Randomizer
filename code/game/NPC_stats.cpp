...
void G_ParseAnimationEvtFile(int glaIndex, const char* eventsDirectory, int fileIndex, int iRealGLAIndex = -1, bool modelSpecific = false)
{
	...
	bool bIsFrameSkipped = (psAnimFileInternalName && strlen(psAnimFileInternalName)>5 && !_stricmp(&psAnimFileInternalName[strlen(psAnimFileInternalName)-5],"_skip"));
...
//			animations[i].initialLerp	= 100;
		...
		//----------------------------
		if (_stricmp(skeletonName, "_humanoid")==0)
		{
	...
	if (modelName)
	{
		...
		//---------------------------------------------------------------------
		if (_stricmp(skeletonName, modelName)!=0)
		{
			const int iGLAIndexToCheckForSkip = (_stricmp(skeletonName, "_humanoid")?-1:gi.G2API_PrecacheGhoul2Model("models/players/_humanoid/_humanoid.gla")); // ;-)
